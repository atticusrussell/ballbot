...
OLED_HEIGHT = 32
SENSOR_INTERVAL = (
    2.0  # seconds: How often to fetch sensor data (Keep it low for efficiency)
)
DISPLAY_UPDATE_INTERVAL = (
    0.5  # seconds: Base refresh rate for display (e.g., for blinking)
)
...
def get_cpu_temperature():
    ...
    try:
        temp_str = subprocess.check_output("vcgencmd measure_temp", shell=True).decode(
            "utf-8"
        )
...
def get_undervoltage():
    ...
    try:
        throttled_str = subprocess.check_output(
...
def system_info(
    device, draw, cpu_temp, ip, cpu_usage, mem_percent, disk_usage, voltage_text
):
    ...
    draw.text(
        (36, 20), disk_usage, fill="white"
    )  # disk_usage is already formatted as "XX%"
    draw.text((70, 20), "MEM:", fill="white")
    draw.text(
        (104, 20), f"{int(mem_percent)}%", fill="white"
    )  # mem_percent is a float from psutil
...
try:
    while True:
        ...
        if cached_undervoltage:
            ...
            voltage_text = (
                "V: OK "  # Ensure trailing space matches "V: LOW" width approx
            )
        ...
        if force_redraw and (
            current_time - last_display_update_time >= DISPLAY_UPDATE_INTERVAL
        ):
        ...
        time_to_next_update = max(
            0, (last_display_update_time + DISPLAY_UPDATE_INTERVAL) - time.time()
        )
        # Also consider time until next sensor read (though usually longer)
        time_to_next_sensor = max(
            0, (last_sensor_update_time + SENSOR_INTERVAL) - time.time()
        )
        ...
        sleep_duration = min(
            time_to_next_update, time_to_next_sensor, DISPLAY_UPDATE_INTERVAL
        )
