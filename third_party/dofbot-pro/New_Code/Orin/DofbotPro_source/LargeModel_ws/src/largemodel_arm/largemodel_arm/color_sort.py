...
M3Pro_demo_pkg="~/yahboomcar_ws/src/M3Pro_demo/M3Pro_demo"
...
class ColorRecognizeNode(Node):
	def __init__(self, name):
		...
		self.done_flag = True
		self.red_hsv_text = os.path.join(M3Pro_demo_pkg,'red_colorHSV.text')
		self.green_hsv_text = os.path.join(M3Pro_demo_pkg,'green_colorHSV.text')
		self.blue_hsv_text = os.path.join(M3Pro_demo_pkg,'blue_colorHSV.text')
		self.yellow_hsv_text = os.path.join(M3Pro_demo_pkg,'yellow_colorHSV.text')
		self.hsv_range = ()
	...
	def get_graspStatusCallBack(self,msg):
		if msg.data == True:
			...
			self.largemodel_arm_done_pub.publish(String(data='color_sort_done'))
	...
	def callback(self,color_frame,depth_frame):
		...
		if key == 32:
			self.statr_flag = True
		if self.cx!=0 and self.cy!=0 and self.circle_r>30 and self.statr_flag == True:
		...
		elif self.statr_flag == True and self.circle_r<30:
			self.statr_flag  = False
			self.pubVel(0,0,0)
			self.Beep_Loop()
			reset = Bool()
