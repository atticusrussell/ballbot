...
from dofbot_pro_msgs.msg import ArmJoint
...
class HandDetector:
    ...
    def fingersUp(self, lmList):
        ...
        point3 = lmList[2][1:3]
        point4 = lmList[1][1:3]
        if (abs(calc_angle(point1, point2, point3)) > 150.0) and (abs(calc_angle(point2, point3, point4)) > 150.0):
            fingers.append(1)
        else:
            fingers.append(0)
        tipIds = [4, 8, 12, 16, 20]
...
class MediaROS(Node):
    def __init__(self):
        ...
        self.pub_image = self.create_publisher(Image, 'Media_image', 10)
...
def main(args=None):
    ...
    media_ros = MediaROS()
    rclpy.spin(media_ros)
    media_ros.destroy_node()
    rclpy.shutdown()
