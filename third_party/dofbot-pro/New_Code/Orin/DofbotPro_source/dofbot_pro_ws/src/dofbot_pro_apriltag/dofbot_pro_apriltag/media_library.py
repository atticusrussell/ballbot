...
import base64
import math
import rclpy  # ROS2使用rclpy库替代rospy
...
from dofbot_pro_interface.msg import *  # 需要确认该消息是否已迁移到ROS2格式
from geometry_msgs.msg import Twist
from cv_bridge import CvBridge  # ROS2需要安装cv-bridge-pkg
...
class Media_ROS:
    def __init__(self):
        ...
        self.pubBuzzer = self.create_publisher(Bool, "Buzzer", 1)
        self.pubCmdVel = self.create_publisher(Twist, 'cmd_vel', 1)
        self.pub_image = self.create_publisher(ImageMsg, 'Media_image', 1)

        # ROS2 订阅者
        self.sub_JoyState = self.node.create_subscription(Bool,'/JoyState',self.JoyStateCallback,1)
