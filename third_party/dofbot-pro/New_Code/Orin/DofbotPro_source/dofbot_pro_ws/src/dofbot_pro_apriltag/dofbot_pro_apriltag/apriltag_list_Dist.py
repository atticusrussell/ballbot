...
class AprilTagDetectNode(Node):
    ...
    def TagDetect(self,color_frame,depth_frame):
        ...
        if len(tags) > 0 and self.done_flag == True:
            for i in range(len(tags)):
                ...
                cv.putText(result_image, str(self.set_dist), (5, 15), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
                ...
                # print(dist_detect)
                if dist_detect<=self.set_dist and self.set_dist!=0 and self.pubPos_flag == True:
            ...
            if self.detect_flag != True and  self.set_dist!=0 and self.pubPos_flag==True:
                print("----------------------------------------")
                self.set_dist!=0
                self.shake()
                time.sleep(2)
    ...
    def shake(self):
        ...
        # beep.data = False
        # self.pub_buzzer.publish(beep)
        self.pub_arm([],1, 60.0, 800)
        time.sleep(0.5)
        self.pub_arm([],1, 120.0, 800)
        time.sleep(0.5)   
        self.pub_arm([],1, 60.0, 800)
        time.sleep(0.5)
        self.pub_arm([],1, 120.0, 800)
        time.sleep(0.5) 
        self.pub_arm([],1, 90.0, 800)
        time.sleep(0.5)      
        
    def GetTargetIDCallback(self,msg):
        print("msg.data: ",msg.data)
        self.set_dist = 170 + msg.data*10
        print("com dist: ",self.set_dist)
