...
class color_detect:
    def __init__(self):
        ...
        self.Center_y_list = []
        self.Center_r_list = []
        # self.pub_joint5 = rospy.Publisher("adjust_joint5", Float32, queue_size=1)
    ...
    def ShapeRecognition(self, img, hsv_msg):
        ...
        for contour in find_contours:
            ...
            if perimeter>200:
                count = count + 1
        #print("count: ",count)
        self.Center_x_list = list(range(count))
        self.Center_y_list = list(range(count))
        #print("self.Center_x_list: ",self.Center_x_list)
        for contour in find_contours:
            perimeter = cv.arcLength(contour, True)
            ...
            if perimeter>200:
                ...
                if shape_name != None:
                    self.Center_x_list[index]= C_x
                    self.Center_y_list[index]= C_y
                    if index<count:
                        index = index + 1
        return img, binary, self.Center_x_list, self.Center_y_list
            

        
    def ShapeDetect(self, img, hsv_msg):
        src = img.copy()
        # 由颜色范围创建NumPy数组
        # Create NumPy array from color range
        src = cv.cvtColor(src, cv.COLOR_BGR2HSV)
        ...
        for contour in find_contours:
            ...
            if perimeter>200:
                ...
                if M['m00'] != 0:
                    ...
                    C_y = int(M['m01'] / M['m00'])
                    cv.circle(img, (C_x,C_y), 10, (0,255,255), thickness=-1)
                    cv.putText(img, shape_name, (C_x+15, C_y+15), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2)
