...
class speech_garbage:
    def __init__(self):
        ...
        self.bus = smbus.SMBus(self.arm.get_i2c_bus_num())
        '''
        ...
        if 1:
            self.bus.write_byte_data(i2c_addr, asr_clear_addr, 0x40)#Clear the power-down buffer area
            self.Busy_Wait()#Wait for the module to be free
            print("Cache cleared")
            self.bus.write_byte_data(i2c_addr, asr_mode_addr, 1)
            self.Busy_Wait()
        ...
        self.bus.write_byte_data(i2c_addr, asr_rec_gain_addr, 0x40)#Set the sensitivity, the recommended value is 0x40-0x55
        self.bus.write_byte_data(i2c_addr, asr_voice_flag, 1)#Set switch sound
        self.bus.write_byte_data(i2c_addr, asr_buzzer, 1)#buzzer
        self.RGBSet(255,255,255)
        time.sleep(1)
        self.RGBSet(0,0,0)
        self.bus.write_byte_data(i2c_addr, asr_buzzer, 0)#buzzer
    ...
    def I2C_WriteBytes(self, str_):
        ...
        for ch in str_:
            try:
                self.bus.write_byte(i2c_speech_addr,ch)
                time.sleep(0.01)
            except:
    ...
    def GetChipStatus(self):
        ...
        try:
            Read_result = self.bus.read_byte(i2c_speech_addr)
            return Read_result
        except:
            print("I2CRead error")
    ...
    def AsrAddWords(self, idnum,str):
        ...
        print(words)
        for date in words:
            self.bus.write_byte (i2c_addr, date)
            time.sleep(0.03)
    ...
    def RGBSet(self, R,G,B):
        ...
        date.append(B)
        print(date)
        self.bus.write_i2c_block_data (i2c_addr,asr_rgb_addr,date)
    ...
    def I2CReadByte(self, reg):
        ...
        #time.sleep(0.05)
        Read_result = self.bus.read_byte (i2c_addr)
        return Read_result
